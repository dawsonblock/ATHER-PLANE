...
- Gatefix 5 implements a resumable native ViZDoom P1P collector that reset-replays each candidate branch twice, scores fixed A-D populations with frozen checkpoints, runs paired learned-model episodes plus a preregistered H8 mixed oracle-search check, and publishes raw evidence only after complete validation. Local test doubles cannot qualify as real-backend evidence; host GPU measurements remain pending.
